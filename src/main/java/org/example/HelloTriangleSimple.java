...
public class HelloTriangleSimple implements GLEventListener, KeyListener {
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        // model matrix
        {
            ...
            Matrix4f scale = (new Matrix4f()).setToScale(0.5f, 0.5f, 0.5f);
            Matrix4f rotateZ = (new Matrix4f()).setToRotationAxis(diff, 0f, 0f, 1f);
            Matrix4f model = scale.mul(rotateZ);
            modelMatrixPointer.asFloatBuffer().put(model.get(new float[16]));
        }
    ...
    public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
        ...
        // ortho matrix
        Matrix4f ortho = (new Matrix4f()).setToOrtho(-1f, 1f, -1f, 1f, 1f, -1f);
        globalMatricesPointer.asFloatBuffer().put(ortho.get(new float[16]));
